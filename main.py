...
for r in range(i):
    ...
    if c is not None and a:  # we don't want empty lines to be translated otherwise this might cause a type error
        result = translator.translate(c)  # here  we translate it from thai to eng , note : since source language can be
...
newd.save('newthai.docx')  # it must be saved to unique name within its file path as word would not give permission to
